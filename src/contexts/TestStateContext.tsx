...
import { Answer, AnswerType, ClientInfo, RawScores, Percentiles, ScaleKey, TestResult } from '@/types/oca';
...
interface TestStateContextValue {
  clientInfo: ClientInfo | null;
  answers: Answer[];
  currentQuestionIndex: number;
...
const TestStateContext = createContext<TestStateContextValue | null>(null);
...
export function TestStateProvider({ children }: { children: ReactNode }) {
  ...
  const getResult = useCallback((): TestResult | null => {
    ...
    const q22 = state.answers.find(a => a.questionId === 22)?.answer;
    const q197 = state.answers.find(a => a.questionId === 197)?.answer;
  ...
  }, [state, isComplete, calculateRawScores, calculatePercentiles]);
...
export function useTestState() {
  ...
  if (!ctx) {
    throw new Error('useTestState must be used within TestStateProvider');
  }
  return ctx;
}
